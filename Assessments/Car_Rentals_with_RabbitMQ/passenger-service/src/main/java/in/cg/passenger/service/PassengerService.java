...
public class PassengerService {
	...
	private final PassengerEventPublisher passengerEventPublisher;

	public PassengerService(PassengerRepository passengerRepository, CarRentalsFeignClient carRentalsFeignClient,
			PassengerEventPublisher passengerEventPublisher) {
		this.passengerRepository = passengerRepository;
		this.carRentalsFeignClient = carRentalsFeignClient;
		this.passengerEventPublisher = passengerEventPublisher;
	}

	public Passenger savePassenger(Passenger passenger) {
		Passenger savedPassenger = passengerRepository.save(passenger);

		PassengerCreatedEvent event = new PassengerCreatedEvent(savedPassenger.getPassengerId(),
				savedPassenger.getPassengerName(), savedPassenger.getSource(), savedPassenger.getDestination(),
				savedPassenger.getCarId());

		passengerEventPublisher.publishPassengerCreatedEvent(event);
